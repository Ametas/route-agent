...
# И ОТДЕЛЬНО — право needrestart перезапускать службы (2026-09-21). Строки выше недостаточно, она
# делает ровно обратное: в неинтерактивной среде needrestart перестаёт спрашивать и перезапускает
# устаревшие службы САМ. Попади в его список ssh или systemd-logind — установка обрывает
# собственную SSH-сессию, а с `set -e` это значит полунастроенный узел. Ловилось живьём: четыре
# обрыва подряд на шаге apt при 7 ГБ свободной памяти.
#
# Перезапускать во время установки нечего: свой юнит агент поднимает сам в самом конце.
